...
import functools
from util import getProbability, raiseNotDefined
...
def joinFactors(factors: ValuesView[Factor]):
    ...
    for factor in factors:
        conditionSet = factor.conditionedVariables()
        conditioned |= conditionSet - unconditioned
    
    newFactor = Factor(unconditioned, conditioned, list(factors)[0].variableDomainsDict())
    possibleAssignments = newFactor.getAllPossibleAssignmentDicts()
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        conditioned = set(factor.conditionedVariables()) - {eliminationVariable}
        newFactor = Factor(unconditioned, conditioned, factor.variableDomainsDict())
        factorAssignments = factor.getAllPossibleAssignmentDicts()
        newFactorAssignments = newFactor.getAllPossibleAssignmentDicts()
        for newAssign in newFactorAssignments:
            prob = 0
            for assignment in factorAssignments:
                part = True
                for key in newAssign.keys():
                    if newAssign[key] != assignment[key]:
                        part = False
                if part:
                    prob += factor.getProbability(assignment)
            newFactor.setProbability(newAssign, prob)
