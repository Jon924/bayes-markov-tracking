...
def constructBayesNet(gameState: hunters.GameState):
    ...
    all_pos = list(itertools.product(range(X_RANGE), range(Y_RANGE)))
    variableDomainsDict[PAC] = all_pos.copy()
    variableDomainsDict[GHOST0] = all_pos.copy()
    variableDomainsDict[GHOST1] = all_pos.copy()
    variableDomainsDict[OBS0] = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    print(variableDomainsDict[OBS0])
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elimVar in eliminationOrder:
            factorsList, joinedFactor = joinFactorsByVariable(factorsList, elimVar)
            if(len(joinedFactor.unconditionedVariables()) > 1):
                joinedFactor = eliminate(joinedFactor, elimVar)
                factorsList.append(joinedFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        normCopy = self.copy()
        normCopy.normalize()
        for key in normCopy.keys():
            if normCopy[key] > val:
                return key
            val = val - normCopy[key]
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        pastBeliefs = self.beliefs.copy()
        disDict = {}
        for pos in self.allPositions:
            disDict[pos] = self.getPositionDistribution(gameState, pos)
        for pos in self.allPositions:
            sum = 0
            for pastPos in self.allPositions:
                posDis = disDict[pastPos]
                sum += pastBeliefs[pastPos]*posDis[pos]
            self.beliefs[pos] = sum
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """
        self.particles = []
        inEach = self.numParticles // len(self.legalPositions)
    ...
    def getBeliefDistribution(self):
        ...
        for pos in self.legalPositions:
            dis[pos] = float(len([1 for p in self.particles if p == pos])) / self.numParticles 
        return dis
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in set(self.particles):
            dis[pos] *= self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
        if dis.total() == 0:
            self.initializeUniformly(gameState)
        else:
            particles = []
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            dis = self.getPositionDistribution(gameState, particle)
            newParticles.append(dis.sample())
